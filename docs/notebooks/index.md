...
These are static, pre-exported snapshots (`marimo export html`) rather than live notebooks — `parx` depends on Julia and PyTorch, neither of which run inside a browser sandbox, so a fully interactive in-browser version isn't possible here. To run either notebook live and edit it, clone the repo and run:
