...
def __(mo):
    ...
    of `A`), `affine_det`, and `active_neuron_count`. Swap the colormap
...
def __(mo):
    ...
    An identical notebook using the Plotly backend lives at
    `notebooks/demo_plotly.py` (also runnable via
    `uv run notebooks/demo_plotly.py`). The only difference is
    `backend="plotly"` (or simply omitting `backend=`, since it's the
    default) passed to each `parx.viz` call:

    - `plot_partition_2d`, `plot_region_counts`, `plot_halfspaces`,
      `plot_partition_slice`, `plot_partition_projection` return an
      interactive `plotly.graph_objects.Figure` with hover tooltips, instead
      of a static `matplotlib.figure.Figure`.
    - `animate_epochs` returns a Plotly figure with a play/pause button and
      slider, instead of a `matplotlib.animation.FuncAnimation`.
    - The Plotly backend has no extra install requirement — it's a core
      dependency of `parx`.
    """)
    return
