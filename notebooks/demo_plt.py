...
def __(mo):
    ...
    `notebooks/demo_plotly.py`. The only difference is `backend="plotly"`
    (or simply omitting `backend=`, since it's the default) passed to each
    `parx.viz` call:

    - `plot_partition_2d`, `plot_region_counts`, `plot_halfspaces` return an
