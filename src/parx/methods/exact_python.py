...
def _chebyshev_center(
    D: np.ndarray, g: np.ndarray
) -> tuple[np.ndarray | None, float]:
    """Chebyshev centre of ``{x : D x ≤ g}``.

    Returns ``(x_interior, radius)`` or ``(None, 0.0)`` when empty / degenerate.
    Zero-norm rows are filtered (they would make the LP unbounded); a zero-norm
    row with strictly negative ``g[i]`` makes the system infeasible.
    """
    m, n = D.shape
    if m == 0:
